...
#ifdef  CONFIG_HUAWEI_KERNEL
#define TLMMV4_GP_FUNC		0
#define TLMMV4_GP_DRV_2MA	0
#endif
...
void msm_gpio_print_enabled(void)
{
	...
	pintype = &tlmm_v4_pininfo[0];
	/*Remove the dead code here*/
	for (i = 0; i < pintype->num_pins; i++) {
		pin_no = i + pintype->pin_start;

		cfg_reg = TLMMV4_GP_CFG(pintype->reg_base, pin_no);
		inout_reg = TLMMV4_GP_INOUT(pintype->reg_base, pin_no);

		cfg_val = readl_relaxed(cfg_reg);
		inout_val = readl_relaxed(inout_reg) & BIT(GPIO_IN_BIT);
		pull_index = (cfg_val >> TLMMV4_GP_PULL_SHFT) & TLMMV4_GP_PULL_MASK;
		func_index = (cfg_val >> TLMMV4_GP_FUNC_SHFT) & TLMMV4_GP_FUNC_MASK;
		drv_index =  (cfg_val >> TLMMV4_GP_DRV_SHFT) & TLMMV4_GP_DRV_MASK;
		dir_index =  (cfg_val >> TLMMV4_GP_DIR_SHFT) & TLMMV4_GP_DIR_MASK; //1:out, 0:in
		val_index = inout_val;
...
static void msm_gpio_config(unsigned int gpio_num)
{
	...
	cfg_reg = TLMMV4_GP_CFG(pintype->reg_base, gpio_num);
	test = cfg_reg;
    /*configure the nc gpio to input and pulldown*/
	val |= (TLMMV4_PULL_DOWN >> TLMMV4_GP_PULL_SHFT) & TLMMV4_GP_PULL_MASK;
	val |= (TLMMV4_GP_FUNC >> TLMMV4_GP_FUNC_SHFT) & TLMMV4_GP_FUNC_MASK;
	val |= (TLMMV4_GP_DRV_2MA >> TLMMV4_GP_DRV_SHFT) & TLMMV4_GP_DRV_MASK;
	val |= (GPIO_IN_BIT >> TLMMV4_GP_DIR_SHFT) & TLMMV4_GP_DIR_MASK;
	writel_relaxed(val, cfg_reg);
}
